<?php
// +-------------------------------------------------------------------------+
// |  Task Scheduler module - Edit an existing cron task                     |
// +-------------------------------------------------------------------------+
...
define('OK',         "Task {$_POST['description']} Updated");
...
if(is_array($crontab) && $crontab[0] !== 'no crontab for ' . exec('whoami')) {
    ...
    foreach($crontab as $key => $task) {

        // If the current row is the task being edited
        if($task === $_POST['old_cron']) {

            // Set the new task
            $new_cron .= "$minute $hour $day_of_month $month $day_of_week " .
                         "{$_POST['command']} {$_POST['argument']} # " .
                         "{$_POST['description']}" . PHP_EOL;

        }

        // Task is not being edited
        else {
...
// Log the event
$ds->logEvent("Task {$_POST['description']} Updated", TASK_UPDATED);
